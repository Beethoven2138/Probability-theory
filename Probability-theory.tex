...
\begin{enumerate}
    ...
    \item[(c)] Suppose $\mathbb{E}[X]<\infty$. Prove that\[\lim_{\delta\downarrow0}\sup_{\{A\in\mathcal{F}:\mathbb{P}(A)<\delta\}}\mathbb{E}[X\mathbf{1}_A]=0,\]that is, given $\epsilon > 0$ there exists a $\delta >0$ such that for all $A\in\mathbf{A}$ with $\mathbb{P}(A)<\delta$, we have $\mathbb{E}[X\mathbf{1}_A]<\epsilon$.
\end{enumerate}
\end{exercise}
